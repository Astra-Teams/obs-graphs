...
"""
            ...
            pr_body_parts = [
                ...
                workflow_result.summary,
                "\n### Details",
                f"- **Total Changes**: {len(workflow_result.changes)} file operations",
                f"- **Nodes Executed**: {len(workflow_result.node_results)}",
                "\n### Node Results",
            ]
            for node_name, result in workflow_result.node_results.items():
                pr_body_parts.append(f"\n#### {node_name}")
                pr_body_parts.append(
                    f"- Status: {'✅ Success' if result['success'] else '❌ Failed'}"
                )
                pr_body_parts.append(f"- Message: {result['message']}")
                pr_body_parts.append(f"- Changes: {result['changes_count']}")
