...
def test_successful_new_article_creation_workflow(
    ...
    mock_github.create_pull_request.return_value = MagicMock(html_url="http://pr")
    ...
    monkeypatch.setattr("src.api.v1.graph.DependencyContainer", lambda: container)

    with patch("src.api.v1.router.run_workflow_task.delay") as mock_delay:
        try:
            ...
            assert isinstance(
                mock_vault.get_concatenated_content_from_category.call_args.args[1],
                Path,
            )
